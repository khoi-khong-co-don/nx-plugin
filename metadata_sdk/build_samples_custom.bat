...
:: Stop Server Witness, Service Name is defaultMediaServer
:: Service Name for NX Meta is metavmsMediaServer
...
cd ../../
:: Start Server Witness, Service Name is defaultMediaServer
:: Service Name for NX Meta is metavmsMediaServer
for /F "tokens=3 delims=: " %%H in ('sc query "defaultMediaServer" ^| findstr "        STATE"') do (
  if /I "%%H" NEQ "RUNNING" (
   REM Put your code you want to execute here
...
:: Check PATH Server Witness or Server Meta
:: Meta "C:\Program Files\Network Optix\Nx Meta\MediaServer"

    set SERVER_DIR="C:\Program Files\Network Optix\Nx Witness\MediaServer"
    echo on
	...
	if NOT [%SAMPLE%] == [unit_tests] (
		copy %SAMPLE_BUILD_DIR%\%BUILD_TYPE%\*.dll %SERVER_DIR%\plugins\ || @exit /b
	)	
    )
    echo:
    echo Built: %ARTIFACT%
