		...
		namespace analytics {
			namespace stub {
				namespace TrafficPlate {
					...
					void IEventController::setTrafficplateDataInfos(std::vector<std::string> trafficdataInfo)
					{
						LOG("SET DATA START");
						m_trafficdataInfo = trafficdataInfo;
						LOG("SET DATA END");
					}

					std::vector<std::string> IEventController::TrafficplateDataInfos()
					{
						LOG("RETURN DATA");
						return m_trafficdataInfo;
					}
